...
꽤 간단합니다. 그렇죠? 여기에 몇 가지 흥미로운 요소들이 있습니다.
 ...
 우리는 conjured item을 제공자와 새롭게 계약을 체결했습니다. 다음 요구사항이 우리 시스템에 반영되어야 합니다.
 - "Conjured" 아이템은 일반 아이템들 보다 품질이 두배씩 빨리 떨어집니다.
 
UpdateQuality 메소드는 자유롭게 변경을 하거나 새로운 코드를 추가할 수 있습니다. 여전히 올바르게 동작한다면 말이죠.
다만 Item 클래스나 그 속성들은 변경하지 마십시오. 그 코드는 저 모퉁이에 있는 고블린에게 속한 것입니다. 그는 공유 코드의 소유권이 자신에게 있다고 여겨지지 않는다면, 분노해서 당신에게 한 발 쏴버릴 것입니다(만약 당신이 원한다면, UpdateQuality 메소드나 아이템의 속성을 static으로 만드는 것 정도는 할 수 있습니다. 그 정도는 우리가 당신을 보호할 것입니다).
